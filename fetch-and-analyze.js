  ...
  try {
    const res = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=" + GEMINI_API_KEY, {
      ...
      body: JSON.stringify({
        ...
        // خاموش‌کردن حالت Thinking برای هزینه و سرعت بهتر (طبق تصمیم فاز ۲ پلن مهاجرت)
    ...
    parsed.forEach(function (item) {
      if (item.keep === false) {
        rejected.push({ sourceUrl: item.sourceUrl });
        return;
      }
...
async function addRejectedUrls(urls) {
  ...
  const now = Date.now();
  const entries = {};
  urls.forEach(u => { if (u) entries[u] = now; });
  if (Object.keys(entries).length === 0) return;
  await redis.zadd('news:rejected', entries);
}
...
async function main() {
  ...
  try {
    ...
    for (var i = 0; i < toAnalyze.length; i += BATCH) {
      ...
      var batchByUrl = {};
      batch.forEach(function (b) { batchByUrl[b.sourceUrl] = b; });
      batchResult.kept.forEach(function (r) {
        var original = batchByUrl[r.sourceUrl];
        if (original) {
          r.pubDate = original.pubDate;
          r.time = formatPersianTime(original.pubDate) || '';
          r.iranScore = original.iranScore;
        }
      });
